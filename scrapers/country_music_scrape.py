...
# Create a filename with the format 'CountryMusicMM.DD.YY.csv'
file_path = f'/Users/jw/Downloads/CountryMusic{today.strftime("%m.%d.%y")}.csv'
...
try:
    # Fetch posts by hashtag (example: 'countrymusic')
    response = api.public.hashtag(name="countrymusic")
    ...
    while response:
        ...
        if 'itemList' in response_json:
            ...
            for item in response_json['itemList']:
                ...
                create_time_dt = datetime.utcfromtimestamp(create_time)  # Convert to datetime object
                ...
                share_count = stats.get('shareCount', 0)  # Calculate additional metrics
                sound_author = music.get('authorName', '')
                sound_id = music.get('id', 0)
                sound_title = music.get('title', 0)

                # Only include the video if it was created in the last 2 months
        ...
        if cursor:
            # If the cursor hasn't changed (i.e., stuck at the same value), break the loop
            if cursor == last_cursor:
                print(f"Cursor stuck at {cursor}. Breaking the loop.")
                break
            print(f"Getting next items with cursor: {cursor}")
            last_cursor = cursor  # Update the last cursor
            response = api.public.hashtag(id=hashtagId, cursor=cursor)  # Pass cursor explicitly
        else:
            break
...
file_path = f'/Users/jw/Downloads/CountryMusic{today}.csv'
output_path = f'/Users/jw/Downloads/CountryMusic{today}.csv'
